...
class MainHandler(BaseHandler):
	...
	def get(self):
		...
		if not is_uni:
			...
			feed = school_actions.get_feed(uniid)
...
class PostHandler(BaseHandler):
	...
	def post(self):
		...
		if not isuni:
			...
			if posttype is PostType.WALL_POST:
				return school_actions.post_wall(userid,message,groupid=uniid)
			elif posttype is PostType.REPLY_POST:
				postid = self.get_argument('postid',strip=True)
				replyid = self.get_argument('replyid',default = None,strip=True)
				print 'replyid',replyid
				ownerid = self.get_argument('ownerid',default = None,strip=True)
				return school_actions.post_wall(ownerid,message,groupid=uniid,commenter = userid,postid=postid,replyid = replyid)
			else:
				print 'huh?'
				return 500;
	...
	def post(self):
		...
		if not isuni:
			...
			if isreply == "true":
				replyid = self.get_argument('reply_id',strip=True)
				if vote == 'up':
					reply = school_actions.upvote_comment(uniid,uid,postid,replyid)
				else:
					reply = school_actions.downvote_comment(uniid,uid,postid,replyid)

			else:
				if vote == 'up':
					reply = school_actions.upvote_post(uniid,uid,postid)
				else:
					reply = school_actions.downvote_post(uniid,uid,postid)
...
class ReplyLoader(BaseHandler):
	...
	def get(self):
		...
		if not is_uni:
			...
			uniid = self.get_current_user()['School']['University']['$oid']
			replies = school_actions.get_replies(uniid,powner,postid)
