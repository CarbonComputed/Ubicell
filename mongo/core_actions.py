...
def post_wall(db,mc,userid,message,commenter=None, postid=None,replyid = None,depth=1):
	...
	if postid is None:
		...
		fid = ObjectId(userid)  #default writing on own wall
		if commenter != None:
			fid = ObjectId(commenter) #someone writing on my wall

		post = {'_id' : ObjectId(),'FriendId' : fid, 'UserId' : ObjectId(userid), 'Message' : message, 'Time' : datetime.datetime.now(),'Hotness' : hotness,  'Upvotes' : 1, 'Downvotes' : 0,'Upvoters' : [fid],'Downvoters' : []}
		db.user.update({'_id' : ObjectId(userid)},{'$push' : { 'Wall' : post}})
	else:
...
def upvote_post(db,userid,postid):
	...
	try:
		...
		postLock.release()
	check = db.user.find_one({ 'Wall._id' : ObjectId(postid)},{'Wall.$' : 1})
	...
	db.user.update({ 'Wall._id' : ObjectId(postid)},{'$set' : {'Wall.$.Hotness' : hotness,  'Wall.$.Upvotes' : upvotes, 'Wall.$.Downvotes' : downvotes,'Wall.$.Upvoters' : upvoters,'Wall.$.Downvoters' : downvoters}})
	postLock.acquire()
	try:
...
def downvote_post(db,userid,postid):
	print 'test'
	postLock.acquire()
	try:
		while lock_table.get(postid,False) is True:
	...
	db.user.update({ 'Wall._id' : ObjectId(postid)},{'$set' : {'Wall.$.Hotness' : hotness,  'Wall.$.Upvotes' : upvotes, 'Wall.$.Downvotes' : downvotes,'Wall.$.Downvoters' : downvoters,'Wall.$.Upvoters' : upvoters}})
	postLock.acquire()
	try:
...
def get_feed(db,userid,includeMe=True,orderBy=None,numResults=60,startNum =1):
	feed = db.user.aggregate([

		{'$match' :{ '$or': [ {"_id": ObjectId(userid) }, {"Friends._id": ObjectId(userid) } ] } },
		{'$unwind' : "$Wall" },
		{'$sort' :  { "Wall.Hotness" : -1 }},
		{'$limit' : numResults }] )
...
def get_user_feed(db,userid,orderBy=None,numResults=20,startNum =1):
	feed = db.user.aggregate([
...
def search(db,userid,universityid,query,query_type = None):
	query = query.strip()
	queries = query.split()
	print queries
	if len(queries) <= 1:
		query = '^'+query.lower();
		results = db.user.find({'School.University' :  ObjectId(universityid), '$or' : [ {'FirstName' : {'$regex' : query}},{'LastName' : {'$regex' : query}},{'UserName' : {'$regex' : query}}]})
	else:
		firstname = queries[0]
		lastname =  queries[1]
		...
		results = db.user.find({'School.University' :  ObjectId(universityid), '$or' : [ {'FirstName' : {'$regex' : fquery}},{'LastName' : {'$regex' : lquery}},{'UserName' : {'$regex' : query}}]})
	return results
